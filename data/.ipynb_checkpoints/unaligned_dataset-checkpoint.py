...
class UnalignedDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        # D = self.transform_D(D_img)
        transform_list = []
        transform_list += [transforms.ToTensor()]
        f = transforms.Compose(transform_list)
        blur_tensor = f(C_img)
        sharp_tensor = f(D_img)

        crop_indices = transforms.RandomCrop.get_params(
                    blur_tensor, output_size=(256, 256))
        i, j, _, _ = crop_indices

        # test = transforms.Resize(400)(HQ_tensor)
        # test = transforms.RandomCrop(256)(HQ_tensor)
        # test = test.unsqueeze(0).to(device).cuda()
        blur_tensor = transforms.functional.crop(blur_tensor, i, j, 256, 256)
        # blur_tensor = blur_tensor.unsqueeze(0).to(device).cuda()
        sharp_tensor = transforms.functional.crop(sharp_tensor, i, j, 256, 256)
        # AQ_tensor = AQ_tensor.unsqueeze(0).to(device).cuda()

        # print(B.shape)
        # B = self.random_masking(B, self.opt.mask_ratio)
        # print(A.shape)
