...
class GANLoss(nn.Module):
    ...
    def __init__(self, gan_mode, target_real_label=1.0, target_fake_label=0.0,
                 decay_k=.99, min_k=.5, steps_decay = 4000, separate_topk=False):
    ...
    def forward(self, iters, dis_output, t_real, dis_update=True, reduce=True):
        ...
        if isinstance(dis_output, list):
            ...
            return self.loss(iters, dis_output, t_real, dis_update)

    def loss(self, iters, dis_output, t_real, dis_update=True):
        r"""GAN loss computation.
        Args:
        ...
        if iters % self.steps_decay == 0 and iters > 0:
            self.topk_anneal(dis_output.size(-1))
        return loss
